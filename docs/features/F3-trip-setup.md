...
1. `/my-trip`: list own trips; `/my-trip/new`: three questions, one per screen: country, city (or another city in that country), then dates on a range calendar (at most `MAX_TRIP_DAYS`, 7). A custom city's selected country is saved with its destination so same-country saved places appear. The title defaults to "Four days in Kyoto" style and can be renamed. The trip then opens the builder, which has three steps:
