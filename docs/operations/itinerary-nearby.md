...
4. Search meal/activity slots near the **next located outing**, then the previous stop, that date's stay,
   or a provider-resolved destination center. Prioritize lunch, then activity suggestions, then other
   meals; cap at two searches/day and 14/generation. Meals use a 1 km radius; activities use 1.5 km
   walking or 3 km otherwise. These are application heuristics, not Google guarantees.
...
6. Reject closed businesses, duplicate provider IDs (including confirmed saved places), out-of-radius
   ...
   Fit a 45-minute meal (60 for relaxed pace), or up to 60 minutes for an optional activity, bounded by
   the original slot. Check inbound and outbound estimated travel with buffers; keep existing blocks fixed.
...
At most one geographic lookup, seven deduplicated forecast requests, and 14 nearby searches. Each request
...
Dietary keywords and indoor categories are hints, not independently certified facts. This first pass targets
at most two slots/day; it does not guarantee a restaurant for every meal or fill every spare minute.
