...
import { createOpenAIExtractor, ProviderError } from "@reel/ai/real-providers";
...
try {
  ...
  if (transcription.status !== "ok") { console.log(JSON.stringify(transcription, null, 2)); process.exitCode = 1; }
    ...
    const candidates = validated.clues.map(clue => ({ clue, status: "unverified", options: [], selected: null }));
    console.log(JSON.stringify({ ...transcription, validatedClues: validated, candidates, saved: false,
      nextStep: "Use the app Inspiration library to save these unverified candidates. Location verification is deferred." }, null, 2));
  }
} catch (error) {
