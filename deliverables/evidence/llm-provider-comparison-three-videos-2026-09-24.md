...
These plain-text paths identify local, gitignored private artifacts that are absent from a public checkout. They are intentionally not Markdown links; CI validates that local link targets exist. This report republishes aggregate metrics and identifiers only; source transcripts, model response text, reviewer identities and credentials are not copied.
...
### CI reference correction (24 September 2026)

The initial report contained nine Markdown links to gitignored private artifacts. Local validation passed because those files existed on the authoring machine; simulating their absence reproduced nine broken-link errors. Converted these references to plain-text paths while preserving identifiers and aggregate results. No private artifacts were added to Git.
