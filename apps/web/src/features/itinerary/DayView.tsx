...
function PanelMap({ markers, tripId, day, activeId }: { markers: MapMarker[]; tripId: string; day: number; activeId?: string }) {
  return <div className="panel-map">
    {markers.length > 0 ? <PlaceMap renderer="google" markers={markers} height={180} interactive={false} activeId={activeId} /> : <div className="map-placeholder">No mapped stops</div>}
    {!activeId && <Link className="panel-day-map-link" href={`/my-trip/${tripId}/map?day=${day}`}><Icon name="map" size={15} /> Explore this day’s map <Icon name="arrowRight" size={14} /></Link>}
  </div>;
}
