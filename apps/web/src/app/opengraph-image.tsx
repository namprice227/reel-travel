...
// Link preview for the site (Open Graph and Twitter). Text-only so it renders without bundled fonts or photos.
// The day card uses fictional sample stops and says so.

export const alt = "Reel Travel: saved travel links, notes and screenshots become a day-by-day trip";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

const STOPS = [
  { time: "10:00", title: "Sample garden", note: "Hours not checked" },
  { time: "11:20", title: "Sample gallery", note: "Confirmed place" },
...
export default function Image() {
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", background: "#f7f7f4", padding: 64, fontFamily: "sans-serif" }}>
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", width: 620 }}>
          <div style={{ display: "flex", fontSize: 34, fontWeight: 700, color: "#1d4ed8" }}>Reel Travel</div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", marginTop: 28, fontSize: 30, color: "#5f6b7a", lineHeight: 1.35 }}>
              Links, notes and screenshots become confirmed places and days you can edit.
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", marginLeft: 48, flex: 1, justifyContent: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", background: "#ffffff", borderRadius: 24, padding: 32, border: "1px solid #e5e7eb" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", fontSize: 18, color: "#1d4ed8", background: "#eef4fd", padding: "6px 14px", borderRadius: 999 }}>Sample trip</div>
            </div>
            {STOPS.map((stop) => (
              <div key={stop.time} style={{ display: "flex", alignItems: "center", marginTop: 22 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ display: "flex", fontSize: 19, color: stop.note === "Fixed booking" ? "#1d4ed8" : "#8a94a3" }}>{stop.note}</div>
                </div>
              </div>
            ))}
          </div>
