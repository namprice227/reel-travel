...
export function createOpenAIItineraryProvider(options: { apiKey?: string; model?: string; temperature?: number; fetch?: typeof fetch }): ItineraryProvider {
  ...
  return { id: "openai", async generate(request) {
    ...
    const raw = await providerJson("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model, store: false, input: [
        { role: "system", content: request.systemPrompt }, { role: "user", content: JSON.stringify({ ...request.input, ...(request.repair ? { repair: request.repair } : {}) }) },
