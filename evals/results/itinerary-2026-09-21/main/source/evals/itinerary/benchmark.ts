...
export async function benchmarkItinerary(input: PlannerContext, provider: ItineraryProvider, options: { maxAttempts?: 1 | 2 } = {}) {
  ...
  try {
    const { plan } = await generateWithProvider(input, provider, { maxAttempts: options.maxAttempts ?? (provider.id === "baseline" ? 1 : 2),
      onAttempt: attempt => { attempts.push(attempt); response = attempt.response; schemaValid = attempt.schemaValid; } });
