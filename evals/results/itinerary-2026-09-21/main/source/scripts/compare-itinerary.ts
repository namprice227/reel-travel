...
await writeFile(path.join(output,"started.txt"),new Date().toISOString(),{flag:"wx"});
...
if(selected.includes("ollama")){
  ...
  await writeFile(path.join(output,"warmup.json"),JSON.stringify({latencyMs:performance.now()-start,status:warm.status,response:await warm.json()},null,2));
}
let done=0;
