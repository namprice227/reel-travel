...
const rates:Record<string,{input:number;cached:number;output:number}>={openai:{input:0.4,cached:0.1,output:1.6},gemini:{input:0.75,cached:0.075,output:3.75}};
function cost(r:any){
...
const summary=Object.fromEntries([...new Set(rows.map(r=>r.provider))].map(provider=>{
  ...
  const finalScore=(r:any)=>r.scores.at(-1);
...
await writeFile(path.join(dir,"summary.json"),JSON.stringify({calculatedAt:new Date().toISOString(),priceDate:"2026-09-21",summary},null,2)+"\n");
...
for(const r of rows)csv.push([r.provider,r.caseId,r.run,r.feasible,r.accepted,r.scores[0]?.accepted??false,r.accepted&&r.scores.at(-1)?.quality?.useful||false,r.attempts,r.latencyMs.toFixed(1),r.inputTokens??"",r.outputTokens??"",cost(r)??""].join(","));
